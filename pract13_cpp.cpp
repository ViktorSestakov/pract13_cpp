...
void CreateThreadPlayer(int colvo)
{
	for (int i = 0; i < colvo; i++)
	{
		...
		pThread[i] = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)GamePlay, (LPVOID)player, 0, 0);
	...
	bThread = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)BossPlay, (LPVOID)colvo, 0, 0);
}
